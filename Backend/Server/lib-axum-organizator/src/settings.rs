...
/// This will be inside the AppState
#[derive(Deserialize, Clone, Debug)]
#[serde(default)]
pub struct Settings {
    pub name: String,
...
fn get_config_content() -> Option<String> {
    ...
    debug!("App name for config paths: {}", app_name);
    ...
    debug!("Possible config paths: {:?}", possible_paths);
    possible_paths
        .iter()
        ...
        .to_str()
        .and_then(|p| {
            info!("Trying to read config from: {:?}", p);
